...
$kubeNamespace = $ApplicationNamespace
...
else
{
	...
	kubectl create secret generic client-id --from-literal=oauth2_proxy_client_id=$ClientId 
	kubectl create secret generic client-secret --from-literal=oauth2_proxy_client_secret=$ClientSecret 
	kubectl create secret generic cookie-secret --from-literal=oauth2_proxy_cookie_secret=$CookieSecret 
